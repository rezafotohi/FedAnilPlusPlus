...
#		self.block_to_add = None
	...
	# FedAnil++: Arithmetic Encoding: EnCoding
	def arithmetic_entropy_coding(self):
		...
		for var in clusters_and_labels_from_kmedoids:
			...
			AE = pyae.ArithmeticEncoding(count_of_any_elements, save_stages=False)
			encoded_msg, encoder , interval_min_value, interval_max_value = AE.encode(msg=data_to_encode, probability_table=AE.probability_table)

			one_var_dict = {}
			one_var_dict["labels"] = encoded_msg
			one_var_dict["lobj"] = AE
			...
			AE = pyae.ArithmeticEncoding(count_of_any_elements, save_stages=False)
			encoded_msg, encoder , interval_min_value, interval_max_value = AE.encode(msg=data_to_encode, probability_table=AE.probability_table)
			one_var_dict["clusters"] = encoded_msg
			one_var_dict["cobj"] = AE
		...
		self.coded_data_after_ac = coded_data
		return coded_data
	# FedAnil++: Arithmetic Encoding: DeCoding
	def arithmetic_entropy_decoding(self, coded_data=None):
		data_encoded = self.coded_data_after_ac
		data_decoded = None
		try:
			for var in data_encoded:
				data_item = data_encoded[var]
				data_to_encode = data_item["labels"]
				unique_labels_, counts_ = np.unique(data_to_encode, return_counts=True)
				count_of_any_elements = dict(zip(unique_labels_, counts_))
				AE = data_item["lobj"]
				data_decoded, decoder = AE.decode(encoded_msg=coded_data, msg_length=6, probability_table=self.AE.probability_table)
		except:
			pass
		return data_decoded
	# FedAnil++: Homomorphic encryption 
